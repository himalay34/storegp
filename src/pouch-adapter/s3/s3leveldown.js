...
function getStartAfterKey(key) {
  ...
  const keyMinusOne =
    keyMinusOneNum >= 0 ? String.fromCharCode(keyMinusOneNum) + "\uFFFF" : "";
...
class S3Iterator extends AbstractIterator {
  constructor(db, options) {
    ...
    if (self._limit === -1) self._limit = Infinity;
    ...
    if (!nullEmptyUndefined(self._finish)) {
      if (ltgt.upperBoundInclusive(options)) self._test = lte;
      else self._test = lt;
    }

    if (!nullEmptyUndefined(self._start))
      self.startAfter = ltgt.lowerBoundInclusive(options)
        ? getStartAfterKey(self._start)
        : self._start;
  ...
  _next(callback) {
    ...
    if (
      self._done++ >= self._limit ||
      (self.data &&
        self.dataUpto == self.data.length &&
        !self.s3nextContinuationToken)
    )
          ...
          if (err) {
            ...
            if (
              ...
              data.Contents.every(function (x) {
                return self._test(
                  x.Key.substring(self.db.folderPrefix.length, x.Key.length)
                );
              })
            ) {
              listObjects();
    ...
    function fireCallback() {
      ...
      for (;;) {
        index = !self._reverse
          ? self.dataUpto
          : self.data.length - 1 - self.dataUpto;
      ...
      if (self.fetchValues) {
        if (self.data[index].Size === 0) getCallback(null, "");
        else self.db._get(key, { asBuffer: self.valueAsBuffer }, getCallback);
      } else getCallback();
...
class S3LevelDown extends AbstractLevelDOWN {
  constructor(location, opts) {
    ...
    if (opts.s3Obj) {
      this.s3 = new AWS.S3Client(opts.s3Obj);
    } else {
      this.s3 = staticS3;
    }
    //this.s3 = opts.s3 || staticS3
  ...
  _open(options, callback) {
    ...
    // bucket creation not working, so menually creat it first
    /*this.s3.send(new AWS.HeadBucketCommand({ Bucket: this.bucket }), (err,res) => {
      if (err) {
		  console.log(err['$metadata'].httpStatusCode)
        // error, bucket is not found
        if (options.createIfMissing && err['$metadata'].httpStatusCode === 403) {
  ...
  _put(key, value, options, callback) {
    if (nullEmptyUndefined(value)) value = Buffer.from("");
  ...
  _batch(array, options, callback) {
    ...
    function act(action, cb) {
      ...
      const key =
        action.key instanceof Buffer ? action.key : String(action.key);
