...
var DOC_ID = `_local/${process.env.LOCAL_DOC_KEY}` || "_local/_security";
...
var requiresAdminWrapper = securityWrapper.bind(
  null,
  function (userCtx, security) {
    return isIn(userCtx, security.admins);
  }
);
["compact", "putSecurity", "viewCleanup", "createIndex", "deleteIndex"].forEach(
  function (name) {
...
var requiresMemberWrapper = securityWrapper.bind(
  null,
  function (userCtx, security) {
    return isIn(userCtx, security.admins) || isMember(userCtx, security);
  }
);
var requireMemberChangesWrapper = createChangeslikeWrapper(
  requiresMemberWrapper
);
...
["changes", "sync", "replicate.to", "replicate.from"].forEach(function (name) {
...
staticSecurityWrappers.replicate = function (original, args) {
  ...
  args.base =
    args.source instanceof PouchDB ? args.source : new PouchDB(args.source);
...
function httpRequest(db, reqStub) {
  return db.info().then(function (info) {
    ...
    return httpQuery(db, reqStub).then(function (resp) {
